...
measured during \glsfirst{MSRE} operation. However, the technical report 
...
\subsection{Insoluble fission product filtering}
...
In this work, we assumed ideal, constant separation efficiency in the 
...
The latest version, Serpent 2, supports advanced geometries and universe 
...
\paragraph*{Simulation.}\textit{Simulation} class runs a Serpent depletion 
...
\begin{figure}[ht!] % replace 't' with 'b' to 
	\centering
	\includegraphics[width=0.9\textwidth]{ch2/tap_materialflow.pdf}
	\vspace{-0.1in}
	\caption{Schematic for passing material data between fuel processing 
...
\begin{figure}[hbt!] % replace 't' with 'b' to 
	\centering
	\includegraphics[width=0.2\textwidth]{ch2/digraph.png}
	\vspace{-0.1in}
...
\begin{figure}[ht!] % replace 't' with 'b' to \centering
	\centering
	\includegraphics[width=0.98\textwidth]{ch2/saltproc_flowchart.pdf}
	\vspace{-0.15in}
...
The UML diagram (Figure~\ref{fig:saltproc_class}) allows the user to model a 
