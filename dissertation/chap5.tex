...
behavior shown in Figure~\ref{fig:power-curve-ny} would require a very fine 
...
shutdown as predicted by Equation~\ref{eq:time-xe-max}. After power ramps down 
...
to 1.0. After shutdown, $^{135}$I decays to $^{135}$Xe that is not burned up. 
...
\begin{table}[htp!]
	...
	\begin{tabularx}{\textwidth}{p{0.06\textwidth} p{0.03\textwidth} R R R R 
		...
		[$pcm$] \\ \hline
		1 & 0.903 & startup+3       & 3542 & 0.749 & +0.33 & -10  \\
...
I analyzed the neutron spectrum of both reactors to understand the difference 
...
worth of control rods must be sufficient to shut down the reactor throughout 
the transient. Ideally, the reactor is more controllable if major 
safety and operational parameters remain stable and unaffected by the 
substantial power level change.
...
Fuel Temperature Coefficient ($\alpha_{T,F}$) increases slightly during the 
first hour of the transient due to a slight spectrum hardening because the  
$^{135}$Xe concentration peak strengthens the Doppler effect in the fuel salt. 
...
For the case with an inactive gas removal system ($\epsilon_{Xe}=0$), 
$^{135}$Xe 
