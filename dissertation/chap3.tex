...
the gradual poisoning of the core graphite with xenon is not treated here.
...
\begin{figure}[ht!] % replace 't' with 'b' to 
	...
	\caption{The number density of major nuclides during 60 years of reactor 
		operation. Figure reproduced from Rykhlevskii \emph{et al.}  
		\cite{rykhlevskii_modeling_2019}.}
	\label{fig:adens_eq}
\end{figure}
\begin{figure}[t] % replace 't' with 'b' to 
	...
	nuclides during 60 years of the reactor operation. Figure reproduced from 
...
\begin{figure}[htp!] % replace 't' with 'b'         to force it to 
	...
	state (normalized by unit lethargy). Figure reproduced from Rykhlevskii 
	\emph{et al.} \cite{rykhlevskii_modeling_2019}.}
	\label{fig:spectrum}
\end{figure}
\begin{figure}[htp!] % replace 't' with 'b' to force it to 
	...
	\caption{The neutron flux energy spectrum for initial and equilibrium  
		state (normalized by unit lethargy). Figure reproduced from 
		Rykhlevskii \emph{et al.} \cite{rykhlevskii_modeling_2019}.}
...
distribution in the \gls{MSBR} core. The spectral shift does not cause 
significant changes in power nor breeding distribution. Even after 20 years of 
...
The average thorium feed rate increases during the first 500 days of 
...
feedback is an expansion of heated material. When the fuel salt temperature 
...
it. When the graphite temperature increases, the density of graphite 
...
\subsection{The effect of removing fission product from the fuel salt}
...
strong absorbers every 3 days did not necessarily lead to fluctuation in 
...
This chapter introduces the very first version of the open-source \gls{MSR} 
...
composition (where equilibrium is defined as when the number densities of 
major isotopes vary by less than 1\% over several years). A secondary goal has 
been to compare predicted operational and safety parameters (e.g., neutron 
energy spectrum, power and breeding distribution, temperature coefficients of 
reactivity) of the \gls{MSBR} at startup and equilibrium state. A tertiary 
goal has been to demonstrate the benefits of continuous fission product  
removal for thermal \gls{MSR} design.
...
spectral shift did not cause any notable changes in power distribution. The 
